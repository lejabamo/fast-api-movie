...
def get_movies_by_category(category: str = Query(min_length=5, max_length=15)) -> List[Movie]:
    ...
    if not result:
          return JSONResponse(status_code=404, content={'message': "No se encontraron pelis con esta categoria."})
    return JSONResponse(status_code=200,content=jsonable_encoder(result))
...
def update_movie(id: int, movie: Movie)-> dict:
    ...
    result = MovieService(db).get_movies_by_id(id)
    if not result:
        return JSONResponse(status_code=404, content ={'message':'No encotrado'})
